...
class Sonarr
{
     ...
     * @return string
     */
    public function postRelease($guid, $indexerId)
    {
        $uri = 'release';
        $uriData = [
            'guid' => $guid,
	 ...
	 * @return string
	 */
	public function postRelease2($guid, $indexerId, $episodeId)
	{
		$uri = 'release';
		$uriData = [
			'guid' => $guid,
	 ...
	 * @return string
	 */
	public function postReleasePush2($title, $downloadUrl, $downloadProtocol, $publishDate)
	{
		$uri = 'release/push';
		$uriData = [
			'title' => $title,
    ...
    protected function _request(array $params)
    {
        ...
        if ( $params['type'] == 'post' ) {
            ...
            $options['json'] = $params['data'];
           // var_dump($client->post($url, $options));
